...
function CancelTimer({ createdAt }: { createdAt: string }) {
  ...
  if (remaining <= 0) {
    return (
      <div className="flex items-center gap-2 rounded-2xl border border-stone-700/80 bg-stone-800/60 px-3 py-2 text-xs font-semibold text-stone-500">
  ...
  return (
    <div className="flex items-center gap-2 rounded-2xl border border-amber-500/30 bg-amber-500/10 px-3 py-2 text-xs font-semibold text-amber-400">
      <svg viewBox="0 0 20 20" className="h-3.5 w-3.5 shrink-0" fill="none" aria-hidden="true">
        <circle cx="10" cy="10" r="7.5" stroke="currentColor" strokeWidth="1.5" />
        <path d="M10 6v4l2.5 1.5" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" />
      </svg>
...
function PushSubscribeButton({ orderId }: { orderId: string }) {
  ...
  if (state === "unsupported" || state === "subscribed") {
    return state === "subscribed" ? (
      <div className="flex items-center gap-2 rounded-2xl border border-emerald-500/30 bg-emerald-500/10 px-3 py-2 text-xs font-semibold text-emerald-400">
  ...
  return (
    <button
      ...
      className="flex items-center gap-2 rounded-2xl border border-stone-700/80 bg-stone-800/60 px-3 py-2 text-xs font-semibold text-stone-400 transition-all duration-200 hover:bg-stone-800 hover:text-stone-200 disabled:opacity-50"
    >
      {state === "loading" ? (
        <span className="h-3.5 w-3.5 animate-spin rounded-full border border-stone-500 border-t-transparent" />
      ) : (
        "🔔"
      )}
...
function StatusProgress({ status }: { status: string }) {
  if (isPendingConfirmation(status)) {
    return (
      <div className="mt-4 flex items-center gap-3 rounded-2xl border border-amber-500/30 bg-amber-500/10 px-4 py-3">
        <div className="h-5 w-5 animate-spin rounded-full border-2 border-amber-500 border-t-transparent" />
        <div className="text-sm font-semibold text-amber-400">Ожидаем подтверждения заказа</div>
      </div>
    );
  }
  ...
  if (status === "delivered") {
    return (
      <div className="mt-4 flex items-center gap-3 rounded-2xl border border-emerald-500/30 bg-emerald-500/10 px-4 py-3">
        <div className="flex h-6 w-6 items-center justify-center rounded-full bg-emerald-500 text-white">
          <IconCheck />
        </div>
        <div className="text-sm font-semibold text-emerald-400">Спасибо за выбор. Заказ доставлен.</div>
      </div>
    );
  }
  ...
  if (isApprovedStatus(status)) {
    return (
      <div className="mt-4 flex items-center gap-3 rounded-2xl border border-emerald-500/30 bg-emerald-500/10 px-4 py-3">
        ...
        <div className="text-sm font-semibold text-emerald-400">Заказ подтвержден</div>
      </div>
    );
  }
  ...
  return (
    <div className="mt-4 flex items-center gap-3 rounded-2xl border border-red-500/30 bg-red-500/10 px-4 py-3">
      ...
      <div className="text-sm font-semibold text-red-400">Заказ отменен</div>
    </div>
  );
}
...
function DeliveryTracker({
  ...
  if (activeStep < 0) {
    return (
      <div className="mt-3 rounded-2xl border border-stone-700/80 bg-stone-800/40 p-3">
        <div className="text-[11px] font-bold uppercase tracking-[0.12em] text-orange-500">Этапы доставки</div>
        <div className="mt-1 text-sm font-semibold text-stone-300">Ожидаем подтверждения оплаты</div>
        <div className="mt-1 text-xs text-stone-500">{formatEtaText(eta)}</div>
      </div>
    );
  }
            ...
            <div key={step} className="flex items-center gap-2">
              <span
                className={`inline-flex h-6 w-6 shrink-0 items-center justify-center rounded-full text-xs font-bold transition-all duration-300 ${
                    ...
                    : current
                      ? "bg-sky-500 text-white shadow-[0_0_0_5px_rgba(14,165,233,0.2)]"
                      : "bg-stone-700 text-stone-500 ring-1 ring-stone-600"
                }`}
              >
                {done ? <IconCheck className="h-3.5 w-3.5" /> : index + 1}
              </span>
              <span className={`text-sm ${done || current ? "font-semibold text-stone-100" : "text-stone-500"}`}>{step}</span>
            </div>
          );
        })}
      </div>
      <div className="mt-2 text-xs text-stone-500">{status === "delivered" ? "Заказ доставлен" : `ETA: ${formatEtaText(eta)}`}</div>
    </div>
  );
}
...
function historyStatusIcon(status: string) {
  ...
  return <span className="inline-flex h-5 w-5 items-center justify-center rounded-full bg-stone-700 text-xs font-bold text-stone-400">•</span>;
}
    ...
    <main className="min-h-screen bg-stone-950 px-4 pb-[calc(57px+env(safe-area-inset-bottom))] pt-5">
      ...
      {showDeliveredFx && (
        <div className="delivered-overlay pointer-events-none fixed inset-0 z-50 flex items-center justify-center px-6">
          <div className="delivered-card relative w-full max-w-sm overflow-hidden rounded-[28px] border border-emerald-500/30 bg-stone-900/95 p-7 text-center shadow-[0_24px_70px_-24px_rgba(16,185,129,0.5)]">
            <div className="relative mx-auto h-24 w-24">
              <div className="delivered-check-ring absolute inset-0 rounded-full border-4 border-emerald-500/40" />
              <div className="delivered-check-core absolute inset-[14px] flex items-center justify-center rounded-full bg-emerald-500 text-white shadow-[0_12px_30px_-12px_rgba(5,150,105,0.85)]">
                <IconCheck className="h-8 w-8" />
              </div>
            </div>
            <div className="mt-4 text-[24px] font-black leading-tight text-emerald-400">Заказ доставлен</div>
            <div className="mt-1 text-sm font-semibold text-emerald-400/70">Приятного аппетита!</div>
      ...
      {showCanceledFx && (
        <div className="canceled-overlay pointer-events-none fixed inset-0 z-50 flex items-center justify-center px-6">
          <div className="canceled-card relative w-full max-w-sm overflow-hidden rounded-[28px] border border-red-500/30 bg-stone-900/95 p-7 text-center shadow-[0_24px_70px_-24px_rgba(239,68,68,0.5)]">
            ...
            <div className="mt-1 text-sm font-semibold text-red-400/70">Администратор отменил заказ</div>
      ...
      <div className="mx-auto max-w-md space-y-3.5">
        ...
        <div className="sticky top-0 z-30 bg-stone-950 border-b border-stone-800/60 px-4 pt-5 pb-4 -mx-4">
          <div className="flex items-end justify-between gap-2">
            <div>
              ...
              <h1 className="mt-1 text-[2.4rem] font-black tracking-[-0.03em] leading-none text-stone-100">Заказ</h1>
            </div>
            {data && !isArchived && (
              <span className={`mb-1 inline-flex rounded-full border px-3 py-1 text-sm font-semibold ${statusMeta.badgeClassName}`}>
                {statusMeta.label}
              </span>
            )}
          </div>
        ...
        {orderLoading && !data ? (
          <div className="rounded-2xl bg-stone-900 border border-stone-800/80 p-5">
          ...
          <div className="rounded-2xl bg-stone-900 border border-stone-800/80 p-5 text-center">
            ...
            <div className="mt-2 font-semibold text-stone-200">Нет активных заказов</div>
            <div className="mt-1 text-sm text-stone-500">Оформите новый заказ в меню</div>
            <Link href={`/r/${menuSlug}`} className="mt-4 block rounded-2xl bg-orange-500 py-3 text-center text-sm font-bold text-white shadow-[0_6px_20px_rgba(249,115,22,0.35)]">
              В меню
            </Link>
          </div>
        ) : !isArchived ? (
          <>
              ...
              <div className="p-4">
                ...
                <div className="mt-3 rounded-2xl border border-stone-700/80 bg-stone-800/40 p-3">
                  <div className="grid grid-cols-2 gap-x-3 gap-y-2 text-sm">
                    <div className="text-stone-500">Итого</div>
                    <div className="text-right text-base font-black text-amber-400">{formatKgs(data?.totalKgs ?? 0)}</div>
                    <div className="text-stone-500">Плательщик</div>
                    <div className="text-right font-bold break-words text-stone-200">{data?.payerName ?? "—"}</div>
                    <div className="text-stone-500">Оплата</div>
                    <div className="text-right text-stone-300">{paymentMethodLabel(data?.paymentMethod ?? "")}</div>
                    <div className="text-stone-500">Телефон</div>
                    <div className="text-right text-stone-300">{data?.customerPhone ?? "—"}</div>
                    <div className="text-stone-500">Создан</div>
                    <div className="text-right text-stone-300">{data?.createdAt ? new Date(data.createdAt).toLocaleString() : "—"}</div>
                  </div>
                </div>

                <div className="mt-2 rounded-2xl border border-stone-700/80 bg-stone-800/40 px-3 py-2 text-sm text-stone-300">
                  Проход <span className="font-bold text-stone-100">{data?.location?.line ?? "—"}</span>, контейнер <span className="font-bold text-stone-100">{data?.location?.container ?? "—"}</span>
                  {data?.location?.landmark ? <> ({data.location.landmark})</> : null}
                </div>
                {data?.comment ? (
                  <div className="mt-2 rounded-2xl border border-amber-500/20 bg-amber-500/8 px-3 py-2 text-sm text-stone-400">
                    💬 {data.comment}
                  </div>
                ) : null}
              </div>
            ...
            <div className="space-y-3">
              {(data?.items ?? []).map((it) => (
                <div key={it.id} className="rounded-2xl bg-stone-900 border border-stone-800/80 p-3">
                  <div className="flex gap-3">
                    <div className="relative h-14 w-14 shrink-0 overflow-hidden rounded-xl bg-stone-800">
                      <Image src={it.photoUrl} alt={it.title} fill className="object-cover" sizes="56px" />
                    </div>
                    <div className="min-w-0 flex-1">
                      <div className="flex items-start gap-3">
                        <div className="min-w-0 flex-1 font-semibold break-words text-stone-100">{it.title}</div>
                        <div className="shrink-0 whitespace-nowrap rounded-xl border border-amber-500/20 bg-amber-500/10 px-2 py-1 text-xs font-black text-amber-400">
          ...
          <div className="rounded-2xl bg-stone-900 border border-stone-800/80 p-5 text-center">
            <div className="text-2xl">✅</div>
            <div className="mt-2 font-semibold text-stone-200">Заказ завершён</div>
            <div className="mt-1 text-sm text-stone-500">Перенесён в историю</div>
            <Link href={`/r/${menuSlug}`} className="mt-4 block rounded-2xl bg-orange-500 py-3 text-center text-sm font-bold text-white shadow-[0_6px_20px_rgba(249,115,22,0.35)]">
              В меню
            </Link>
          </div>
        ...
        <div className="rounded-2xl bg-stone-900 border border-stone-800/80 overflow-hidden">
          <button className="flex w-full items-center justify-between px-4 py-4 text-left" onClick={() => setHistoryOpen((value) => !value)}>
            <div className="flex items-center gap-2">
              <span className="inline-flex h-6 w-6 items-center justify-center rounded-full bg-stone-800 text-stone-400">
                <IconHistory className="h-4 w-4" />
              </span>
              <span className="text-sm font-semibold text-stone-300">История заказов</span>
            </div>
            <div className="flex items-center gap-3">
              <span className="text-xs text-stone-500">{history.length}</span>
              <IconChevron open={historyOpen} className="h-4 w-4 text-stone-500" />
            </div>
          </button>
            ...
            <div className="border-t border-stone-800 px-4 pb-4 pt-3">
              <div className="space-y-2">
                {history.map((order) => {
                  ...
                  return (
                    <div key={order.id} className="rounded-2xl border border-stone-700/80 bg-stone-800/40">
                      <button
                      ...
                      >
                        ...
                        <div className="flex-1 text-center text-sm font-bold text-stone-200">{formatKgs(order.totalKgs)}</div>
                        <div className="w-32 shrink-0 text-right">
                          <div className="text-xs text-stone-500">{createdDate.toLocaleDateString()}</div>
                          <div className="text-xs text-stone-500">{createdDate.toLocaleTimeString()}</div>
                          <div className="mt-0.5 inline-flex items-center gap-1 text-[11px] font-semibold text-stone-500">
                      ...
                      {isExpanded && (
                        <div className="motion-fade-up border-t border-stone-700/80 px-3 pb-3 pt-2">
                          ...
                          {order.comment ? <div className="mt-1 text-xs text-stone-500">💬 {order.comment}</div> : null}

                          <div className="mt-3 space-y-2">
                            {order.items.map((item) => (
                              <div key={item.id} className="flex items-center gap-2 rounded-xl border border-stone-700/80 bg-stone-800/60 p-2">
                                ...
                                <div className="text-sm font-bold text-amber-400">{formatKgs(item.priceKgs * item.qty)}</div>
                              </div>
                            ))}
                          </div>
                        </div>
                ...
                {history.length === 0 && <div className="py-2 text-center text-sm text-stone-500">История заказов пока пуста.</div>}
              </div>
            </div>
          )}
        </div>
